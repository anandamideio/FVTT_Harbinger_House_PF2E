...
import {
	PF2E_BESTIARY_TOKENS_MODULE_ID,
...
export class HarbingerHouseImporter extends foundry.applications.sheets.AdventureImporter {
	...
	async _onImport(importResult: Record<string, unknown>, importOptions: Record<string, boolean>) {
		...
		await this.#safeStep('normalizeSceneBestiaryTokenArt', () => this.#normalizeSceneBestiaryTokenArt());
	...
	/**
	 * Replace token texture and subject paths that point at the optional Bestiary
	 * token module when that module is inactive.
	 */
	async #normalizeSceneBestiaryTokenArt() {
			...
			if (bestiaryModuleActive) {
				this.#debug('Skipping Bestiary token normalization because token module is active');
				return;
			}
			...
			this.#debug('Normalized scene token Bestiary art paths', {
